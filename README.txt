...
The files are used as following:

config.json
Contains all general info
...
config.json and config_clearText.txt are intended to be used in parallel.
...
- for Common Fund Stash (Shturman's Stash), WeaponBox5x5 (Green) and WeaponBox6x3 (Big grey) the min and max item value that can spawn in that container
...
- debugMode_showDebugInfo in combination with "debugMode_containerIndex" allows to see all the items in a certain container
- selectedItems_AdjustSpawnChance allows to adjust the spawn rate of defined items
- favoriteItems contains all the items that have a slightly higher initial spawn chance
...
Changelog v1.0.6 (w.i.p.):
- complete rework of the mod - meanwhile an "awesome" spawn behavior and hopefully a much better readability (if you want to adjust stuff in the code yourself)
- user set parameters moved into different .json configuration files
- added config parameter "equalSpawnratesForAllItems" to make all handbook items spawn with the same chance (include Secure containers/Pouches with parameter "equalSpawnratesForAllItemsExcludePouches" set to false)
- added config parameter "equalSpawnratesForAllKeys" to make all mechanical keys spawn with the same chance. To avoid adjusting the rate_KEYS_MECHANICALKEYS value whenever you have this option turned on, I added an additional parameter "equalSpawnratesForAllKeysMulti"
- added section "favoriteItems", any item listed in this section will get a higher initial spawn rate
- added the option for the high-tier containers (Shturman's Stash, 5x5 Weaponbox, 6x3 Weaponbox) to spawn items either on the single item value or per slot value (to avoid e.g. 10 slot weapons being excluded the mod will always add all weapons).
- spawn rates get some variation after each raid
- independent spawn rates per container - for now it's only used to give keys in jackets a different spawn rate compared to keys in drawers (parameter: keysInJacketsMultiplier)
- option to enable/disable multiple item spawns of same type in one container (parameter: allowDuplicates)
- container "JacketDorms114" now has much less items (10) with equal spawn chance, to increase looting Dorm room 114 key
- debug info will now show you the minimum, maximum and median spawn rate and the info how many items of each category are included in the container
